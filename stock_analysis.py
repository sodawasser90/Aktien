...
def plot_stock_data(data, ticker):
    ...
    ax.plot(data['Close'], label=f'Aktienkurs von {ticker}')
    ax.set_title(f'Aktienkursverlauf von {ticker}')
    ax.set_xlabel('Datum')
    ax.set_ylabel('Preis')
    ax.legend()
    st.pyplot(fig)
...
def plot_moving_averages(data, ticker):
    fig, ax = plt.subplots(figsize=(12,6))
    ax.plot(data['Close'], label='Schlusskurs', color='black')
    ax.plot(data['SMA50'], label='50-Tage-SMA', color='blue')
    ax.plot(data['SMA200'], label='200-Tage-SMA', color='red')
    ax.set_title(f'Gleitende Durchschnitte von {ticker}')
    ax.set_xlabel('Datum')
    ax.set_ylabel('Preis')
    ax.legend()
    st.pyplot(fig)
...
def main():
    ...
    if choice == "Aktie suchen":
        ...
        recommendations = {
            "AAPL": ("Apple Inc.", "Kaufsignal"),
            "MSFT": ("Microsoft Corp.", "Beobachten"),
            "GOOGL": ("Alphabet Inc.", "Kaufsignal")
        }
